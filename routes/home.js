...
router.post("/recentKey", isAuth, async (req, res) => {
  try {
    ...
    if (!recentKey) {
      ...
      if (recentKey.keyword.length < 10) {
        //배열이 10개 미만일때
        if (recentKey.keyword.includes(keyword)) {
          //키워드가 이미 리스트에 존재할때
